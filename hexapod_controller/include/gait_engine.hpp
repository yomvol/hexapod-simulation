...
class GaitEngine {
 public:
  explicit GaitEngine(int cycle_duration);

  /// @brief Computes one point of the given leg's trajectory, if the computed
  /// point is unreachable, returns std::nullopt
  ...
  /// @param leg_id The ID of the leg (0-5)
  /// @param time The time at which to compute the IK
  /// @param foot_pos_global Position of the foot in body frame, pass this
  /// argument for visualization purposes
  /// @return The joint angles for the leg
