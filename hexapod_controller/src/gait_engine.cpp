...
Vec3 GaitEngine::computeLegFK(const std::array<double, 3>& joint_angles) {
  ...
  T0_1 = dhToTransform(joint_angles[0] + dh_params_.theta1_offset,
                       dh_params_.d1, dh_params_.a1, dh_params_.alpha1);
  T1_2 = dhToTransform(joint_angles[1] + dh_params_.theta2_offset,
                       dh_params_.d2, dh_params_.a2, dh_params_.alpha2);
  T2_3 = dhToTransform(joint_angles[2] + dh_params_.theta3_offset,
                       dh_params_.d3, dh_params_.a3, dh_params_.alpha3);
  T0_3 = T0_1 * T1_2 * T2_3;
...
std::optional<std::array<double, 3>> GaitEngine::computeLegIK(Vec3 leg_tip_position) {
  leg_tip_position.z += Z_OFFSET;  // add offset to the Z coordinate, because IK
